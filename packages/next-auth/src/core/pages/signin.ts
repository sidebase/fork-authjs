import { h, type CSSProperties, type VNode, type VNodeChild } from "vue"
import type { InternalProvider, Theme } from "../types"

/**
 * The following errors are passed as error query parameters to the default or overridden sign-in page.
 *
 * Documentation: https://next-auth.js.org/configuration/pages#sign-in-page
 */
export type SignInErrorTypes =
  | "Signin"
  | "OAuthSignin"
  | "OAuthCallback"
  | "OAuthCreateAccount"
  | "EmailCreateAccount"
  | "Callback"
  | "OAuthAccountNotLinked"
  | "EmailSignin"
  | "CredentialsSignin"
  | "SessionRequired"
  | "default"

export interface SignInServerPageParams {
  csrfToken: string
  providers: InternalProvider[]
  callbackUrl: string
  email: string
  error: SignInErrorTypes
  theme: Theme
}

function hexToRgba(hex?: string, alpha = 1) {
  if (!hex) {
    return
  }

  // Remove the "#" character if it's included
  hex = hex.replace(/^#/, "")

  // Expand 3-digit hex codes to their 6-digit equivalents
  if (hex.length === 3) {
    hex = hex[0] + hex[0] + hex[1] + hex[1] + hex[2] + hex[2]
  }

  // Parse the hex value to separate R, G, and B components
  const bigint = parseInt(hex, 16)
  const r = (bigint >> 16) & 255
  const g = (bigint >> 8) & 255
  const b = bigint & 255

  // Ensure the alpha value is within the valid range [0, 1]
  alpha = Math.min(Math.max(alpha, 0), 1)

  // Construct the RGBA string
  return `rgba(${r}, ${g}, ${b}, ${alpha})`
}

function renderThemeStyle(theme: Theme): VNode[] {
  const nodes: VNode[] = []

  if (theme.brandColor) {
    nodes.push(
      h("style", {
        innerHTML: `
:root {
  --brand-color: ${theme.brandColor};
}
        `.trim(),
      })
    )
  }

  if (theme.buttonText) {
    nodes.push(
      h("style", {
        innerHTML: `
:root {
  --button-text-color: ${theme.buttonText};
}
        `.trim(),
      })
    )
  }

  return nodes
}

export default function SigninPage(props: SignInServerPageParams) {
  const {
    csrfToken,
    providers,
    callbackUrl,
    theme,
    email,
    error: errorType,
  } = props

  // We only want to render providers
  const providersToRender = providers.filter((provider) => {
    if (provider.type === "oauth" || provider.type === "email") {
      // Always render oauth and email type providers
      return true
    } else if (provider.type === "credentials" && provider.credentials) {
      // Only render credentials type provider if credentials are defined
      return true
    }
    // Don't render other provider types
    return false
  })

  const errors: Record<SignInErrorTypes, string> = {
    Signin: "Try signing in with a different account.",
    OAuthSignin: "Try signing in with a different account.",
    OAuthCallback: "Try signing in with a different account.",
    OAuthCreateAccount: "Try signing in with a different account.",
    EmailCreateAccount: "Try signing in with a different account.",
    Callback: "Try signing in with a different account.",
    OAuthAccountNotLinked:
      "To confirm your identity, sign in with the same account you used originally.",
    EmailSignin: "The e-mail could not be sent.",
    CredentialsSignin:
      "Sign in failed. Check the details you provided are correct.",
    SessionRequired: "Please sign in to access this page.",
    default: "Unable to sign in.",
  }

  const error = errorType ? (errors[errorType] ?? errors.default) : undefined
  const providerLogoPath = "https://authjs.dev/img/providers"

  return h("div", { class: "signin" }, [
    ...renderThemeStyle(theme),

    h("div", { class: "card" }, [
      theme.logo
        ? h("img", {
            src: theme.logo,
            alt: "Logo",
            class: "logo",
          })
        : null,

      error
        ? h("div", { class: "error" }, [h("p", error)])
        : null,

      ...providersToRender.map((provider, i): VNodeChild => {
        let bg = ""
        let text = ""
        let logo = ""
        let logoDark = ""
        let bgDark = ""
        let textDark = ""

        if (provider.type === "oauth") {
          ;({
            bg = "",
            text = "",
            logo = "",
            bgDark = bg,
            textDark = text,
            logoDark = "",
          } = provider.style ?? {})

          logo = logo.startsWith("/") ? `${providerLogoPath}${logo}` : logo
          logoDark = logoDark.startsWith("/")
            ? `${providerLogoPath}${logoDark}`
            : logoDark || logo

          logoDark ||= logo
        }

        const shouldRenderTopHr =
          (provider.type === "email" || provider.type === "credentials") &&
          i > 0 &&
          providersToRender[i - 1].type !== "email" &&
          providersToRender[i - 1].type !== "credentials"

        const shouldRenderBottomHr =
          (provider.type === "email" || provider.type === "credentials") &&
          i + 1 < providersToRender.length

        return h("div", { key: provider.id, class: "provider" }, [
          provider.type === "oauth"
            ? h(
                "form",
                { action: provider.signinUrl, method: "POST" },
                [
                  h("input", {
                    type: "hidden",
                    name: "csrfToken",
                    value: csrfToken,
                  }),

                  callbackUrl
                    ? h("input", {
                        type: "hidden",
                        name: "callbackUrl",
                        value: callbackUrl,
                      })
                    : null,

                  h(
                    "button",
                    {
                      type: "submit",
                      class: "button",
                      style: {
                        "--provider-bg": bg,
                        "--provider-dark-bg": bgDark,
                        "--provider-color": text,
                        "--provider-dark-color": textDark,
                        "--provider-bg-hover": hexToRgba(bg, 0.8),
                        "--provider-dark-bg-hover": hexToRgba(bgDark, 0.8),
                      } as CSSProperties,
                    },
                    [
                      logo
                        ? h("img", {
                            loading: "lazy",
                            height: 24,
                            width: 24,
                            id: "provider-logo",
                            src: logo,
                          })
                        : null,

                      logoDark
                        ? h("img", {
                            loading: "lazy",
                            height: 24,
                            width: 24,
                            id: "provider-logo-dark",
                            src: logoDark,
                          })
                        : null,

                      h("span", `Sign in with ${provider.name}`),
                    ]
                  ),
                ]
              )
            : null,

          shouldRenderTopHr ? h("hr") : null,

          provider.type === "email"
            ? h(
                "form",
                { action: provider.signinUrl, method: "POST" },
                [
                  h("input", {
                    type: "hidden",
                    name: "csrfToken",
                    value: csrfToken,
                  }),

                  h(
                    "label",
                    {
                      class: "section-header",
                      for: `input-email-for-${provider.id}-provider`,
                    },
                    "Email"
                  ),

                  h("input", {
                    id: `input-email-for-${provider.id}-provider`,
                    autofocus: true,
                    type: "email",
                    name: "email",
                    value: email,
                    placeholder: "email@example.com",
                    required: true,
                  }),

                  h(
                    "button",
                    {
                      id: "submitButton",
                      type: "submit",
                    },
                    `Sign in with ${provider.name}`
                  ),
                ]
              )
            : null,

          provider.type === "credentials" && provider.credentials
            ? h(
                "form",
                { action: provider.callbackUrl, method: "POST" },
                [
                  h("input", {
                    type: "hidden",
                    name: "csrfToken",
                    value: csrfToken,
                  }),

                  ...Object.keys(provider.credentials).map((credential) => {
                    const config = provider.credentials![credential]

                    return h(
                      "div",
                      { key: `input-group-${provider.id}-${credential}` },
                      [
                        h(
                          "label",
                          {
                            class: "section-header",
                            for: `input-${credential}-for-${provider.id}-provider`,
                          },
                          config.label ?? credential
                        ),

                        h("input", {
                          name: credential,
                          id: `input-${credential}-for-${provider.id}-provider`,
                          type: config.type ?? "text",
                          placeholder: config.placeholder ?? "",
                          ...config,
                        }),
                      ]
                    )
                  }),

                  h("button", { type: "submit" }, `Sign in with ${provider.name}`),
                ]
              )
            : null,

          shouldRenderBottomHr ? h("hr") : null,
        ])
      }),
    ]),
  ])
}
